...
const showNotification = ref(localStorage.getItem('banacanvas-show-notification') === 'true')
...
function toggleNotification() {
  ...
  localStorage.setItem('banacanvas-show-notification', String(showNotification.value))
}
...
<template>
  <div class="flex flex-col gap-5">
    ...
    <!-- Show System Notification Toggle -->
    <div class="flex items-center justify-between">
      ...
      <button
        ...
        :class="[
          'relative inline-flex h-5 w-9 shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none focus:ring-2 focus:ring-violet-500 focus:ring-offset-2 dark:focus:ring-offset-gray-900',
          showNotification ? 'bg-violet-600' : 'bg-gray-300 dark:bg-gray-700',
        ]"
      >
        <span
          :class="[
            'pointer-events-none inline-block h-4 w-4 transform rounded-full bg-white shadow ring-0 transition duration-200 ease-in-out',
            showNotification ? 'translate-x-4' : 'translate-x-0',
          ]"
        />
      </button>
    </div>
